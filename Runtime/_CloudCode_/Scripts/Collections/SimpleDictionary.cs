...
namespace Nextension
{
    public class SimpleDictionary<TKey, TValue> : ICollection<KeyValuePair<TKey, TValue>>
    {
        ...
        public TValue this[TKey key]
        {
            ...
            set
            {
                __insert(key, value, add: false);
            }
        }

        public ref TValue GetAsRef(TKey key)
        {
            int num = FindEntry(key);
            if (num >= 0)
            {
        ...
        private void __checkNullKey(TKey key)
        {
            if (key is null)
            {
        ...
        private void __initialize(uint capacity)
        {
        ...
        private void __insert(TKey key, TValue value, bool add)
        {
            __checkNullKey(key);
        ...
        public int EnsureCapacity(uint capacity)
        {
            ...
            __resize(HashHelpers.expandPrime(capacity), forceNewHashCodes: false);
            return entries.Length;
        }

        private void __resize()
        {
            __resize(HashHelpers.expandPrime((uint)count), forceNewHashCodes: false);
        }

        private void __resize(int newSize, bool forceNewHashCodes)
        {
