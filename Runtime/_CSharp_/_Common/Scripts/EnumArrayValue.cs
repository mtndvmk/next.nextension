...
namespace Nextension
{
    ...
    public class EnumArrayValue<TEnum, TValue> : IEnumArrayValue, IEnumerable<TValue>, ISerializationCallbackReceiver where TEnum : unmanaged, Enum
    {
        ...
        public void set(TEnum enumType, TValue val)
        {
            if (this == Empty)
            {
                NDebug.LogWarning("Cannot set value to Empty");
                return;
            }
        ...
        public void clear()
        {
            Array.Clear(enumValues, 0, enumValues.Length);
        }
        public void copyTo(EnumArrayValue<TEnum, TValue> other)
        {
        ...
        public struct Enumerator : IEnumerator<KeyValuePair<TEnum, TValue>>
        {
            ...
            public bool MoveNext()
            {
                ...
                {
                    _current = new KeyValuePair<TEnum, TValue>(EnumIndex<TEnum>.getEnum(_index), _enumValues[_index]);
                    _index++;
                    return true;
                }
                return false;
