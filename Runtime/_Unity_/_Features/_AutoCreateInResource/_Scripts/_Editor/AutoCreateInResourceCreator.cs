...
namespace Nextension.NEditor
{
    internal class AutoCreateInResourceCreator : IOnCompiled, IOnAssetImported
    {
        ...
        static async void onLoadOrRecompiled()
        {
            ...
            foreach (var type in types)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        NDebug.LogWarning($"Warning: Cannot create `{type.FullName}`, reason: {ex}");
                    }
                    else
                    {
                        NDebug.LogError($"Failed to create `{type.FullName}`, reason: {ex}");
                    }
                    continue;
                }
            }
        ...
        static void onAssetDeleted(string path)
        {
            ...
            {
                NDebug.LogWarning($"Did you delete AutoCreateOnResource file?, please don't do that!\nPath: {path}");
                onLoadOrRecompiled();
            }
        }
    }
