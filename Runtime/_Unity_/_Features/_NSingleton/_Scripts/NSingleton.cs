            ...
            else
            {
                ...
                else
                {
                    if (s_Instance != this)
                    {
                        ...
                        {
                            Destroy(gameObject);
                            NDebug.Log("Destroyed " + typeof(T).Name + ", IsInitialized: " + IsInitialized);
                            return;
                        }
                    }
                }
        ...
        private async void OnDestroy()
        {
            if (s_Instance == this)
            {
                if (IsPlaying)
                {
                    if (!_isDestroyedFromInternal)
                    {
                        if (!m_DontDestroyOnLoad)
                        {
                            ...
                            for (int i = 0; i < sceneCount; i++)
                            {
                                ...
                                {
                                    NDebug.LogWarning($"[NSingleton] Should call {getSingletonName()}.Instance.destroy()");
                                    return;
                                }
                            }
                        }
                    ...
                    {
                        NDebug.Log($"{Instance.gameObject.name} was destroyed");
                    }
                    onDestroy();
                }
                else
            ...
            get
            {
                if (s_Instance.isNull())
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            NDebug.LogWarning($"Failed to initialize NSingleton<{typeof(T)}>: " + e.Message);
                        }
                        else
                        {
                            NDebug.LogError($"Failed to initialize NSingleton<{typeof(T)}>: " + e.Message);
                        }
                    }
                }
                return s_Instance;
    ...
    public static class S_<T> where T : class, ISingletonable
    {
        ...
        public static T get()
        {
            if (s_Instance.isNull())
            {
                ...
                {
                    s_Instance = NUtils.createInstance<T>();
                }
            }
            return s_Instance;
        }
