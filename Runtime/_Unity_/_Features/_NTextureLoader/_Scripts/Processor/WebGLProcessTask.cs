...
namespace Nextension.TextureLoader
{
        ...
        internal async NTaskVoid exeCompleteData(WebGLProcessCompleteData completeData)
        {
            try
            {
                if (string.IsNullOrEmpty(completeData.error))
                {
                    ...
                    {
                        var nativeBytes = NUtils.convertToNativeArray<byte>(completeData.DataPtr, completeData.length, Allocator.TempJob);
    ...
    internal class NTextureProcessWebGLCallback : MonoBehaviour
    {
        ...
        private void onProcessCompleteCallback(string jsonResult)
        {
            ...
            if (result == null)
            {
                NDebug.LogError("Has error when decode image WebGL");
                return;
            }
        ...
        private void loadTextureAtNativeCompleteCallback(string jsonResult)
        {
            ...
            if (result == null)
            {
                NDebug.LogError("Has error when load into texture");
                return;
            }
