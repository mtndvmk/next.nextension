...
namespace Nextension
{
    ...
    public static class InsPool
    {
        ...
        public static void release(GameObject gameObject)
        {
            ...
            {
                NDebug.LogError($"{gameObject.name} is not from pool");
            }
            else
            {
                origin.release();
        ...
        public static SharedInsPool<T> getSharedPool<T>(this Object prefab) where T : Object
        {
        ...
        where T : Object
    {
        ...
        private T createNewInstance(Vector3? position = null)
        {
            T ins;
            if (position.HasValue)
            {
                ins = Object.Instantiate(getPrefab(), position.Value, Quaternion.identity, InsPoolContainer.Container);
            }
            else
            {
                ins = Object.Instantiate(getPrefab(), InsPoolContainer.Container, true);
            }
        ...
        {
            return __getInternal(null, parent, worldPositionStays);
        }
        public override T get(Vector3 position, Transform parent = null, bool worldPositionStays = true)
        {
            return __getInternal(position, parent, worldPositionStays);
        }
        private T __getInternal(Vector3? position, Transform parent, bool worldPositionStays)
        {
            requireCall();
            if (_disableSharedPool)
            {
                ...
                {
                    ins = createNewInstance(position);
                    isNewInstance = true;
                }
            ...
            return position.HasValue
                ? InsPoolUtil.getInstanceFromGO<T>(SharedPool.get(position.Value, parent, worldPositionStays))
                : InsPoolUtil.getInstanceFromGO<T>(SharedPool.get(parent, worldPositionStays));
        }
        ...
        public override void release(T instance)
        {
            ...
            if (_disableSharedPool)
            {
                if (origin.IsInPool)
                {
                    NDebug.LogWarning($"Instance has been in pool", instance);
                    return;
                }
                if (origin.Id != Id)
                {
                    NDebug.LogWarning($"OriginId [{origin.Id}], [{Id}] not match, try release by SharedInsPool", go);
