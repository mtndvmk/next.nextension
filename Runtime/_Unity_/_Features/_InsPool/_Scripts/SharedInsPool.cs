...
namespace Nextension
{
    public ref struct SharedInsPool<T> where T : Object
    {
        ...
        public readonly T get(Vector3 position, Transform parent = null, bool worldPositionStays = true)
        {
            return InsPoolUtil.getInstanceFromGO<T>(SharedInsPoolUtil.getPool(_poolId).get(position, parent, worldPositionStays));
        }
        ...
        public readonly T getAndRelease(IWaitable waitable, Vector3 position, Transform parent = null, bool worldPositionStays = true)
        {
            var pool = SharedInsPoolUtil.getPool(_poolId);
            var go = pool.get(position, parent, worldPositionStays);
            pool.waitAndRelease(waitable, go);
            return InsPoolUtil.getInstanceFromGO<T>(go);
        }
        ...
        public readonly T getAndDelayRelease(float delaySeconds, Vector3 position, Transform parent = null, bool worldPositionStays = true)
        {
            var pool = SharedInsPoolUtil.getPool(_poolId);
            var go = pool.get(position, parent, worldPositionStays);
            pool.waitAndRelease(new NWaitSecond(delaySeconds), go);
        ...
        {
            return new ComponentInstantiator<T>(getGameObjectInstantiator());
        }
        public readonly ComponentInstantiator<T1> getComponentInstantiator<T1>() where T1 : Object
        {
            return new ComponentInstantiator<T1>(getGameObjectInstantiator());
        }
    }
}