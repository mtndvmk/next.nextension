...
namespace Nextension
{
    public class GuiCommandUIManager : MonoBehaviour
    {
        ...
        private List<GuiCmdUIData> _logData = new List<GuiCmdUIData>();
        private List<GuiCmdUIData> _savedData = new List<GuiCmdUIData>();
        ...
        private void Start()
        {
            ...
            _runButton.onButtonClickEvent.AddListener(__runCmdFromIF);
        ...
        private void __runCmdFromIF()
        {
            if (_cmdIF.text.Length > 0)
            {
                var cmds = _cmdIF.text.Split('\n');
        ...
        private void __runCmd(string cmdInput)
        {
            ...
            var cmdResult = GuiCommand.runCmd(cmdInput);
            var results = cmdResult.results;
            if (results != null && results.Count > 0)
            {
                cmdUIData.results = new List<object>(results);
            }
        ...
        public void addSavedCmd(string cmdInput, bool canDelete)
        {
            ...
            if (index >= 0)
            {
                var saveCmdData = _savedData[index];
                if (!canDelete && saveCmdData.checkTag == GuiCmdUIData.Saved_CanDelete)
                {
                    saveCmdData.checkTag = GuiCmdUIData.Saved_NotDelete;
                    _guiCmdUIView.getShowingCell(index)?.refreshCellData();
                }
                return;
            }
        ...
        public void removeSavedCmd(string cmdInput)
        {
            __removeSaveCmd(cmdInput);
        }
