...
namespace Nextension
{
    public class GuiCommand : NSingleton<GuiCommand>
    {
        ...
        [SerializeField] private List<string> _defaultCommands = new List<string>();

        private static Dictionary<string, NAction<CommandData>> _cmdListeners = new Dictionary<string, NAction<CommandData>>();

        public static readonly Dictionary<string, string> CommandInfos = new Dictionary<string, string>();
        ...
        private static void __invokeCmd(CommandData cmdData)
        {
            if (_cmdListeners.TryGetValue(cmdData.CmdName, out var listeners) && listeners.Count > 0)
            {
                foreach (var listener in listeners)
                {
                    ...
                    {
                        cmdData.results.Add(e);
                    }
                }
            }
            else
            {
                cmdData.results.Add(new Exception("Invalid command"));
            }
        }
        ...
        public static void registerAsDefault(string cmdName, Action<CommandData> action)
        {
            register(cmdName, action);
            Instance._uiManager.addSavedCmd(cmdName, false);
        }
        ...
        public static CommandData runCmd(string cmdInput)
        {
    ...
    {
        public List<string> cmdInputs = new List<string>();
    }
}
