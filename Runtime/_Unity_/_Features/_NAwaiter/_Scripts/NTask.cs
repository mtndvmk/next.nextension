...
namespace Nextension
{
    ...
    public readonly struct NTask
    {
        ...
        public readonly void cancel()
        {
            if (!IsCreated || !IsInProgress)
            {
                NDebug.LogWarning($"Can not cancel NTask[{id}]: not exists or not in progress");
                return;
            }
            NTaskManager.cancel(id);
        }
        ...
        public static async NTask whenAll(NTask a, NTask b)
        {
            await a;
            await b;
        }
    ...
    public readonly struct NTask<T>
    {
        ...
        public readonly void cancel()
        {
            if (!IsCreated || !IsInProgress)
            {
                NDebug.LogWarning($"Can not cancel NTask<{typeof(T)}>[{id}]: not exists or not in progress");
                return;
            }
