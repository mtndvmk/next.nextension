        ...
        private static void manualImport()
        {
            ...
            if (validScriptables.Count > 0)
            {
                ...
                foreach (var scriptableObject in validScriptables.asSpan())
                {
                    ...
                    {
                        NDebug.Log($"Added [{scriptableObject}] to SingletonScriptableContainer", scriptableObject);
                        addCount++;
                    }
                    else
                    {
                        NDebug.LogWarning($"[{scriptableObject}] is added or is not SingletonScriptable", scriptableObject);
                    }

                }

                NDebug.Log($"Added {addCount} SingletonScriptables", container);
        ...
        public class OnLoadOrRecompiled : IOnCompiled, IOnAssetImported
        {
            ...
            static async void onAssetImported(string path)
            {
                ...
                if (ScriptableLoader.isSingletonable(scriptableObject))
                {
                    ...
                    if (!container)
                    {
                        NDebug.LogError($"Missing {EditorSingletonScriptableContainer.FileNameInResource} on Resource directory");
                        return;
                    }
                    await TaskEditor.waitFrame();
                    if (container.add(scriptableObject))
