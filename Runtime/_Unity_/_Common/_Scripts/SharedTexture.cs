...
namespace Nextension
{
    public static class SharedTextureManager
    {
        ...
        private static void checkOrigins()
        {
            foreach (var origin in origins)
            {
                ...
                {
                    NDebug.LogWarning("Texture of OriginTexture has been destroy without dispose?");
                }
            }
        }
#endif
        internal static HashSet<OriginTexture> origins = new HashSet<OriginTexture>();
    ...
    public class OriginTexture
    {
        ...
        private HashSet<SharedTexture> _sharedList = new HashSet<SharedTexture>();
        ...
        public void release(SharedTexture sharedTexture, bool isDisposeIfNoLongerUsed = true)
        {
            if (IsDisposed)
            {
                NDebug.LogWarning("OriginTexture has been disposed");
                return;
            }
    ...
    public class SharedTexture
    {
        ...
        public void release(bool isDisposeIfNoLongerUsed)
        {
            if (IsReleased)
            {
                NDebug.LogWarning("SharedTexture has been released");
                return;
            }
            if (origin.IsDisposed)
            {
