...
{
    [BurstCompile]
    public static class BurstEaseFloat3
    {
        [BurstCompile]
        ...
        [BurstCompile]
        public static void lerp(in float3 a, in float3 b, in float t, out float3 result)
        {
            result = math.lerp(a, b, math.saturate(t));
        }
        [BurstCompile]
        public static void sineIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, 1 - math.cos(ts * math.PI / 2));
        }
        [BurstCompile]
        public static void sineOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, math.sin(ts * math.PI / 2));
        }
        [BurstCompile]
        public static void sineInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, (math.cos(ts * math.PI) - 1) / -2);
        }
        [BurstCompile]
        public static void quadIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, ts * ts);
        }
        [BurstCompile]
        public static void quadOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = 1 - ts;
        ...
        [BurstCompile]
        public static void quadInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = -2 * ts + 2;
        ...
        [BurstCompile]
        public static void cubicIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, ts * ts * ts);
        }
        [BurstCompile]
        public static void cubicOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = 1 - ts;
        ...
        [BurstCompile]
        public static void cubicInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = -2 * ts + 2;
        ...
        [BurstCompile]
        public static void quartIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, ts * ts * ts * ts);
        }
        [BurstCompile]
        public static void quartOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = 1 - ts;
        ...
        [BurstCompile]
        public static void quartInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = -2 * ts + 2;
        ...
        [BurstCompile]
        public static void quintIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, ts * ts * ts * ts * ts);
        }
        [BurstCompile]
        public static void quintOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = 1 - ts;
        ...
        [BurstCompile]
        public static void quintInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = -2 * ts + 2;
        ...
        [BurstCompile]
        public static void expoIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, math.pow(2, 10 * ts - 10));
        }
        [BurstCompile]
        public static void expoOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, 1 - math.pow(2, -10 * ts));
        }
        [BurstCompile]
        public static void expoInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
        ...
        [BurstCompile]
        public static void circIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, 1 - math.sqrt(1 - ts * ts));
        }
        [BurstCompile]
        public static void circOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = ts - 1;
        ...
        [BurstCompile]
        public static void circInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = 2 * ts;
            float t2 = -2 * ts + 2;
        ...
        [BurstCompile]
        public static void backIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, 2.70158f * ts * ts * ts - 1.70158f * ts * ts);
        }
        [BurstCompile]
        public static void backOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = ts - 1;
            float t2 = t1 * t1;
        ...
        [BurstCompile]
        public static void backInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float t1 = ts * 2;
            float t2 = t1 - 2;
        ...
        [BurstCompile]
        public static void elasticIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, -(math.pow(2, 10 * ts - 10) * math.sin((ts * 10 - 10.75f) * (2 * math.PI) / 3)));
        }
        [BurstCompile]
        public static void elasticOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            result = math.lerp(a, b, math.pow(2, -10 * ts) * math.sin((ts * 10 - 0.75f) * (math.PI * 2) / 3) + 1);
        }
        [BurstCompile]
        public static void elasticInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            float ts = math.saturate(t);
            float f = ts < 0.5f ?
        ...
        [BurstCompile]
        public static void bounceOut(in float3 a, in float3 b, in float inT, out float3 result)
        {
        ...
        [BurstCompile]
        public static void bounceIn(in float3 a, in float3 b, in float t, out float3 result)
        {
            bounceOut(0, 1, 1 - t, out var b1);
            result = math.lerp(a, b, 1 - b1);
        }
        [BurstCompile]
        public static void bounceInOut(in float3 a, in float3 b, in float t, out float3 result)
        {
            bounceOut(0, 1, 1 - 2 * t, out var b1);
            bounceOut(0, 1, 2 * t - 1, out var b2);
            float3 f = t < 0.5f ? ((1 - b1) / 2) : ((1 + b2) / 2);
            result = math.lerp(a, b, f);
        }
    }
}
