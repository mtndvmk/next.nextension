...
namespace Nextension
{
    ...
    public class NReadOnlyAttribute : ApplyToCollectionPropertyAttribute
    {
        ...
        public NReadOnlyAttribute() : base()
        {
            order = NAttributeOrder.READONLY;
            predicateName = null;
            conditionValues = null;
        }
        public NReadOnlyAttribute(string predicateName) : base()
        {
            if (predicateName.isNullOrEmpty())
            {
                NDebug.LogError($"`{nameof(predicateName)}` is null or empty");
                return;
            }
        ...
        public NReadOnlyAttribute(string predicateName, object conditionValue) : base()
        {
            if (predicateName.isNullOrEmpty())
            {
                NDebug.LogError($"`{nameof(predicateName)}` is null or empty");
        ...
        public NReadOnlyAttribute(string predicateName, params object[] conditionValues) : base()
        {
            if (predicateName.isNullOrEmpty())
            {
                NDebug.LogError($"`{nameof(predicateName)}` is null or empty");
